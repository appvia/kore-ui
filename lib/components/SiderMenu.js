import Link from "next/link"
import { Layout, Menu, Icon } from 'antd'
const { Sider } = Layout
const { SubMenu } = Menu

class SiderMenu extends React.Component {
  state = {
    siderCollapsed: false
  }

  onSiderCollapse = siderCollapsed => {
    this.setState({
      siderCollapsed
    })
  }

  render() {
    if (this.props.hide) {
      return null
    }

    const menuItem = ({ key, text, href, link, icon }) => (
      <Menu.Item key={key} style={{margin: '0'}}>
        <Link href={href || link} as={link}>
          <a className="collapsed"><Icon type={icon} />{text}</a>
        </Link>
      </Menu.Item>
    )

    const AdminMenu = () => this.props.isAdmin ? (
      <SubMenu key="configure"
        title={
          <span>
            <Icon type="tool" />
            <span>Configure</span>
          </span>
        }
      >
        {menuItem({ key: 'configure_providers', text: 'Cloud providers', link: '/configure/cloud-providers', icon: 'cloud' })}
        {menuItem({ key: 'configure_plans', text: 'Cluster templates', link: '/admin/cluster-templates', icon: 'copy' })}
        {menuItem({ key: 'configure_settings', text: 'Settings', link: '/admin/settings', icon: 'setting' })}
      </SubMenu>
    ) : null

    return (
      <Sider
        collapsible
        collapsed={this.state.siderCollapsed}
        onCollapse={this.onSiderCollapse}
        width="235"
      >
        <Menu defaultOpenKeys={['configure', 'teams', 'spaces']}  mode="inline">
          {AdminMenu()}
          <SubMenu key="teams"
            title={
              <span>
                <Icon type="team" />
                <span>Teams</span>
              </span>
            }
          >
            {menuItem({ key: 'new_team', text: 'New team', link: '/teams/new', icon: 'plus-circle' })}
            {(this.props.teams || []).map(t => (
              menuItem({ key: t.metadata.name, text: t.spec.summary, href: '/teams/[name]', link: `/teams/${t.metadata.name}`, icon: 'team' })
            ))}
          </SubMenu>
          <SubMenu key="spaces"
            title={
              <span>
                <Icon type="appstore" />
                <span>Spaces</span>
              </span>
            }
          >
            {menuItem({ key: 'new_space', text: 'New space', link: '/spaces/new', icon: 'plus-circle' })}
          </SubMenu>
        </Menu>
      </Sider>
    )
  }
}

export default SiderMenu
