    ...
    return this.props.form.validateFields(async (err, values) => {
      if (!err) {
        try {
          ...
          const existingNc = await apiRequest(null, 'get', `/teams/${this.props.team}/namespaceclaims/${resourceName}`)
          ...
          const nsClaimResult = await apiRequest(null, 'put', `/teams/${this.props.team}/namespaceclaims/${resourceName}`, nsClaimResource)
          this.props.form.resetFields()
          const state = copy(this.state)
          state.submitting = false
          this.setState(state)
