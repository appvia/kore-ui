...
class ClusterBuildForm extends React.Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields(async (err, values) => {
      if (!err) {
        ...
        const resourceName = this.generateClusterName()
  ...
  generateClusterName = () => {
    let clusterName = this.state.selectedPlan ? `${this.state.selectedProvider.spec.class}-${this.state.selectedBinding}-${this.props.team.metadata.name}-${this.state.selectedPlan.metadata.name}` : ''
    const matchingClusterCount = this.props.teamClusters.filter(tc => tc.metadata.name.indexOf(clusterName) >=0 ).length
          ...
          {selectedPlan ? (
            <div>
              <Form.Item label="Cluster name">
                <Text>{this.generateClusterName()}</Text>
              </Form.Item>
              <Alert
                ...
                description={
                  <List
                    size="small"
                    dataSource={this.props.teamClusters}
                    renderItem={cluster => (
                      <List.Item>
