...
class ClusterBuildForm extends React.Component {
  ...
  rawClusterName = () => this.state.selectedPlan ? `${this.state.selectedProvider.spec.class}-${this.state.selectedBinding}-${this.props.team.metadata.name}-${this.state.selectedPlan.metadata.name}` : ''
                ...
                {this.props.providers.map(provider =>
                  provider.spec.bindings.map(binding =>
                    provider.spec.resources.map(resource => (
                      <Option key={`${provider.spec.class}|${binding}|${resource.kind}`} value={`${provider.spec.class}|${binding}|${resource.kind}`}>{provider.spec.displayName} ({binding}) - {resource.displayName}</Option>
                    ))
                  )
                )}
              </Select>
