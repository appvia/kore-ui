...
class OrgService {
  ...
  async getTeamMembers(team, requestingIdToken) {
    try {
      const result = await axios.get(`${this.koreApi.url}/teams/${team}/members`, { headers: this.getHeaders(requestingIdToken) })
  ...
  async getUserTeams(username, requestingIdToken) {
    try {
      const result = await axios.get(`${this.koreApi.url}/users/${username}/teams`, { headers: this.getHeaders(requestingIdToken) })
      return result.data.items
    } catch (err) {
  ...
  async getTeamGkeCredentials(team, requestingIdToken) {
    try {
      const result = await axios.get(`${this.koreApi.url}/teams/${team}/gkecredentials`, { headers: this.getHeaders(requestingIdToken) })
      return result.data
    } catch (err) {
